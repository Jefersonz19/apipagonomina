package apipagonomina.apipagonomina.servicios;
...
public class FechaServicio implements IFechaServicio {
    ...
    public Date calcularFechaPago(Date fecha) {
        ...
        Date proximoPago = getProximoPago(fecha);

        while(!esDiahabil(proximoPago)) {
            proximoPago = substractDays(proximoPago, 1);
        }
        return proximoPago;
    }

    public Date getProximoPago(Date fecha) {
        ...
        if (dayOfMonth <= 15) {
            return new Date(year - 1900, month, 15);
        } else {
            return new Date(year - 1900, month, 30);
        }
    }
    ...
    private Date substractDays(Date fecha, int dias){
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(fecha);
    ...
    private boolean isWeekend(Date fecha) {
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(fecha);
